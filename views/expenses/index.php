...
<link rel="stylesheet" href="<?php echo constant('URL') ?>public/css/history.css">
    ...
    <script>
        ...
        function filterByCategory(value){
            this.copydata = [...this.data];
            const res = this.copydata.filter(item =>{
                return value == item.nameCategory;
            });
            this.copydata = [...res];
            renderData(res);
        }
        ...
        async function getData(){
            data = await fetch('http://localhost/CursoPHP/ExpesesAPP/expenses/getHistoryJSON')
            .then(res =>res.json())
            .then(json => json);
            this.copydata = [...this.data];
            console.table(data);
        ...
        function renderData(data){
            ...
            data.forEach(item => { 
                ...
                databody.innerHTML += `<tr>
                        <td>${item.title}</td>
                        <td><span class="category" style="background-color: ${item.color}">${item.nameCategory}</span></td>
                        <td>${item.date}</td>
                        <td>$${item.amount}</td>
                        <td><a href="http://localhost/CursoPHP/ExpesesAPP/expenses/delete/${item.expensesId}">Eliminar</a></td>
                    </tr>`;
            });
        }
